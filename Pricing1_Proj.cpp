...
// Import the "Curves.h" file to create a Piecewise Linear Curve of volatilities.
...
public:
    ...
    // Initialize an empty vector of European Options.
    ...
    void ThreadFuction(
        ...
        for (int mc_i = 0; mc_i < paths_per_thread; ++mc_i) {
            ...
            if (step == true) {
                ...
                for (int it_options = 0; it_options < num_options; it_options++) {
                    ...
                    // variables (according to the first algorothm).
                    ws->setDiff(
                        payoffs_args[it_options], eos[it_options].total_price - eos[it_options].obs_price
                    );
                ...
                for (int it_options = 0; it_options < num_options; it_options++) {
                    ...
                    grad_vector[it_options][mc_i] = aadc::mmSum(
        ...
        if (step == true) {
            ...
            for (int it_options = 0; it_options < num_options; it_options++) {
                ...
                grad_total[it_options] = aadc::mmSum(mm_grad[it_options])/num_mc_paths;
    ...
    void operator()(const VectorXd& vol_list_obs) {
        ...
        ScalarVector price_obs(num_options, 0.)
        ...
        ThreadFuction(vol_list_obs, true, grad_vector, price_obs, grad_total);
        
        // Store the results for the observed prices for each European Option.
        for (int it = 0; it < num_options; it++) eos[it].obs_price = price_obs[it];
    ...
    double operator()(const VectorXd& x, VectorXd& grad) {
        ...
        ScalarMatrix grad_vector(num_options, paths_per_thread, 0.);

        // With "true", calculate only the expectations of the payoffs.
        ThreadFuction(x, true, grad_vector, exp_price, grad_total);
        
        // Store the results for the expectations for each European Option.
        for (int it_options = 0; it_options < num_options; it_options++) {
...
private:
    ...
    int num_options = 5, num_mc_paths = 10000000, AVXsize = aadc::mmSize<mmType>()
...
void option_pricing_1_Proj()
{

    // Initializes the number of iterations. 
    int iterations_1P;
    std::cout << "Number of iterations: ";
    std::cin >> iterations_1P;

    // Sets up parameters.
    LBFGSParam<double> param_1P;
    param_1P.epsilon = 1e-9;
    param_1P.max_iterations = iterations_1P;

    // Creates the solver and function object.
    LBFGSSolver<double> solver_1P(param_1P);
    ...
    int niter = solver_1P.minimize(fun, x, fx);
