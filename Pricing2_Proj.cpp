            ...
            //the gradient of G
                ...
                if (mc_i > 0){
                    ...
                    for (int it_options = 0; it_options < num_options; it_options++){
                        ...
                        int index = (std::rand() % (mc_i));

                        ws->setDiff(
                            payoffs_args[it_options], eos[it_options].payoffs[index] - eos[it_options].obs_price
                        );
...
private:
    ...
    int num_options = 5 //Number of European options
        , num_mc_paths = 10000000 //Number of Monte-Carlo simulations
...
{   
    ...
    std::cout << "Base time (double): " << 0 << " iterations = " << base_time.count() << " microseconds\n\n";
